...
describe("JetStakingV2", function () {
    ...
    it('should test multiple stakers reward calculation', async () => {
        ...
        await auroraToken.connect(streamManager).approve(jetV2.address, auroraProposalAmountForAStream)
        ...
        await auroraToken.connect(user2).approve(jetV2.address, amount2)
        ...
        await jetV2.connect(user2).stake(amount2)
        ...
        await jetV2.connect(streamManager).proposeStream(
            user1.address,
            streamToken1.address,
        ...
        await jetV2.connect(user1).createStream(id, maxRewardProposalAmountForAStream)
        await network.provider.send("evm_setAutomine", [false])

        // After 1 year (1st year rewards)
        await jetV2.connect(user1).moveRewardsToPending(id)
        await jetV2.connect(user2).moveRewardsToPending(id)
        ...
        const user1Pending1 = await jetV2.getPending(id, user1.address)
        const user2Pending1 = await jetV2.getPending(id, user2.address)
        ...
        const user1Pending2 = await jetV2.getPending(id, user1.address)
        const user2Pending2 = await jetV2.getPending(id, user2.address)
        ...
        const user1Pending3 = await jetV2.getPending(id, user1.address)
        const user2Pending3 = await jetV2.getPending(id, user2.address)
        await network.provider.send("evm_setAutomine", [true])
    ...
    it('should test multiple stakers compound reward during 6 months', async () => {
        ...
        // After 6 months
        await jetV2.connect(user1).unstakeAll()
        await jetV2.connect(user2).unstakeAll()
        await network.provider.send("evm_mine", [startTime + oneYear])
        const firstYearRewards = scheduleRewards[0].sub(scheduleRewards[1])
        const user1Pending = await jetV2.getPending(id, user1.address)
        const user2Pending = await jetV2.getPending(id, user2.address)
        await network.provider.send("evm_setAutomine", [true])
    ...
    it('should test multiple stakers compound reward during 1 year', async () => {
        ...
        // After 6 months
        await auroraToken.connect(user3).approve(jetV2.address, amount3)
        await jetV2.connect(user3).stake(amount3)
        await network.provider.send("evm_mine", [startTime + oneYear])

        const totalShares = await jetV2.totalAuroraShares()

        // After 1 year (1/2 year with 2 stakers + 1/2 year with 3 stakers)
        await jetV2.connect(user1).unstakeAll()
        await jetV2.connect(user2).unstakeAll()
        await jetV2.connect(user3).unstakeAll()
        await network.provider.send("evm_mine", [startTime + oneYear + oneYear / 2])
        const firstYearRewards = scheduleRewards[0].sub(scheduleRewards[1])
        const secondYearRewards = scheduleRewards[1].sub(scheduleRewards[2])
        const user1Pending = await jetV2.getPending(id, user1.address)
        const user2Pending = await jetV2.getPending(id, user2.address)
        const user3Pending = await jetV2.getPending(id, user3.address)
        await network.provider.send("evm_setAutomine", [true])
    ...
    it("should return treasury account", async () => {
        expect(await jetV2.treasury()).to.eq(treasury.address)
    })
    ...
    it('should allow admin to propose new stream', async () => {
        ...
        await expect(jetV2.connect(streamManager).proposeStream(
            user1.address,
            streamToken1.address,
            auroraProposalAmountForAStream,
            maxRewardProposalAmountForAStream,
    ...
    it('should allow stream owner to create a stream', async () => {
        ...
        await streamToken1.connect(user1).approve(jetV2.address, maxRewardProposalAmountForAStream)
        // create a stream
        const tx = await jetV2.connect(user1).createStream(id, maxRewardProposalAmountForAStream)
        const {streamId, owner, } = await getEventLogs(tx.hash, constants.eventsABI.streamCreated, 0)
        expect(owner).to.be.eq(user1.address)
        expect(streamId.toNumber()).to.be.eq(id)
        expect(await jetV2.getStreamsCount()).to.be.eq(2)
    })
    it('should refund stream owner when stream created with less rewards', async () => {
        ...
        // approve reward tokens
        await streamToken1.connect(user1).approve(jetV2.address, minRewardProposalAmountForAStream)
        // create a stream and refund half to stream manager without rounding error.
        await expect(jetV2.connect(user1).createStream(id, minRewardProposalAmountForAStream))
            .to.emit(auroraToken, "Transfer").withArgs(jetV2.address, streamManager.address, 1)
    })
    it('should create stream and refund staking admin if deposit reward is less than the upper amount', async () => {
        ...
        await streamToken1.connect(user1).approve(jetV2.address, RewardProposalAmountForAStream)
        // create a stream
        await jetV2.connect(user1).createStream(id, RewardProposalAmountForAStream)
        const stream = await jetV2.getStream(id)
        ...
        const rewardSchedule = await jetV2.getStreamSchedule(id)
    ...
    it('should release aurora rewards to stream owner', async () => {
        ...
        await network.provider.send("evm_mine")
        const claimableBefore = await jetV2.getStreamOwnerClaimableAmount(id)
        await jetV2.connect(user1).releaseAuroraRewardsToStreamOwner(id)
        ...
        const claimableAfter = await jetV2.getStreamOwnerClaimableAmount(id)
    ...
    it('should stake aurora tokens', async () => {
        ...
        await auroraToken.connect(user1).approve(jetV2.address, amountStaked)
        const tx = await jetV2.connect(user1).stake(amountStaked)
    ...
    it('should not release new rewards in the same block', async () => {
        ...
        await auroraToken.connect(user1).approve(jetV2.address, amount)
        await jetV2.connect(user1).stake(amount)
        const touchedAt = await jetV2.touchedAt()
        const rewards = await jetV2.getRewardsAmount(0, touchedAt)
    ...
    it('user stakes and never claims', async () => {
        ...
        await auroraToken.connect(user1).approve(jetV2.address, amountStaked1)
        await jetV2.connect(user1).stake(amountStaked1)
        const currentTime = (await ethers.provider.getBlock("latest")).timestamp
        const timeDiff = currentTime - startTime
        await network.provider.send("evm_increaseTime", [4 * oneYear - timeDiff - 1])
        await network.provider.send("evm_mine")
        await jetV2.connect(user1).updateUserCalculation()
        ...
        expect(parseInt(ethers.utils.formatEther(await jetV2.totalAmountOfStakedAurora()))).to.be.lessThanOrEqual(
            parseInt(expectedReward.toString())
        )
        expect(parseInt(ethers.utils.formatUnits(amountStaked1, 0))).to.be.eq(
            parseInt(ethers.utils.formatUnits(await jetV2.totalAuroraShares(), 0))
        )
    })
    it('should able to get schedule times per stream', async () => {
        const schedules = await jetV2.getStreamSchedule(0)
        expect(schedules[0][0]).to.be.eq(scheduleTimes[0])
    })
    it('should be able to get reward per share', async () => {
        expect(
            ethers.utils.formatEther(await jetV2.getRewardPerShare(0))
        ).to.be.eq("0.0")
    })
    it('should schedule from 0 to 4 years', async () => {
        const {total, rewardPerShareAurora, scheduleCalculated} = await jetV2.before(scheduleTimes[0], scheduleTimes[4])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0], scheduleTimes[4])
    ...
    it('should schedule from 1 to 2 years', async () => {
        const { total, rewardPerShareAurora, scheduleCalculated } = await jetV2.before(scheduleTimes[1], scheduleTimes[2])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[1], scheduleTimes[2])
    ...
    it('should schedule from 1 to 3', async () => {
        const { total, rewardPerShareAurora, scheduleCalculated } = await jetV2.before(scheduleTimes[1], scheduleTimes[3])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[1], scheduleTimes[3])
    ...
    it('should schedule from 0 to 1', async () => {
        const { total, rewardPerShareAurora, scheduleCalculated } = await jetV2.before(scheduleTimes[0], scheduleTimes[1])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0], scheduleTimes[1])
    ...
    it('should schedule from 0 to now (200 days)', async () => {
        ...
        const { total, rewardPerShareAurora, scheduleCalculated } = await jetV2.before(scheduleTimes[0], (await ethers.provider.getBlock("latest")).timestamp)
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0], (await ethers.provider.getBlock("latest")).timestamp)
        const timeDiff = currentTime - startTime
        const expectedScheduledReward = 
    ...
    it('should schedule from 200 to now (750 days)', async () => {
        ...
        const currentTime = (await ethers.provider.getBlock("latest")).timestamp
        const timeDiff = currentTime - startTime
        const { total, rewardPerShareAurora, scheduleCalculated } = await jetV2.before(scheduleTimes[0] + 200 * oneDay, (await ethers.provider.getBlock("latest")).timestamp)
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, startTime, currentTime)
    ...
    it('should schedule from 200 to end (4 years)', async () => {
        const {total, rewardPerShareAurora, scheduleCalculated} = await jetV2.before(scheduleTimes[0] + 200 * oneDay, scheduleTimes[4])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0] + 200 * oneDay, scheduleTimes[4])
        const expectedScheduledReward1 = (parseInt(ethers.utils.formatEther(scheduleRewards[0])) - parseInt(ethers.utils.formatEther(scheduleRewards[1]))) * (oneYear - 200 * oneDay) / oneYear
        const expectedScheduledReward2 = (parseInt(ethers.utils.formatEther(scheduleRewards[1])) - parseInt(ethers.utils.formatEther(scheduleRewards[2]))) + 
                (parseInt(ethers.utils.formatEther(scheduleRewards[2])) - parseInt(ethers.utils.formatEther(scheduleRewards[3]))) +
    ...
    it('should schedule from 200 to end (3 years)', async () => {
        const {total, rewardPerShareAurora, scheduleCalculated} = await jetV2.before(scheduleTimes[0] + 200 * oneDay, scheduleTimes[3])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0] + 200 * oneDay, scheduleTimes[3])
        const expectedScheduledReward1 = (parseInt(ethers.utils.formatEther(scheduleRewards[0])) - parseInt(ethers.utils.formatEther(scheduleRewards[1]))) * (oneYear - 200 * oneDay) / oneYear
        const expectedScheduledReward2 = (parseInt(ethers.utils.formatEther(scheduleRewards[1])) - parseInt(ethers.utils.formatEther(scheduleRewards[2]))) + 
            (parseInt(ethers.utils.formatEther(scheduleRewards[2])) - parseInt(ethers.utils.formatEther(scheduleRewards[3])))
        const expectedScheduledReward = parseInt(expectedScheduledReward1.toString()) + parseInt(expectedScheduledReward2.toString())
    ...
    it('should schedule from 400 to end (3 years)', async () => {
        const {total, rewardPerShareAurora, scheduleCalculated} = await jetV2.before(scheduleTimes[0] + 400 * oneDay, scheduleTimes[3])
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0] + 400 * oneDay, scheduleTimes[3])
    ...
    it('should schedule from 400 to end of (3rd year) + 2 day', async () => {
        const {total, rewardPerShareAurora, scheduleCalculated} = await jetV2.before(scheduleTimes[0] + 400 * oneDay, scheduleTimes[3] + 2 * oneDay)
        const { startIndex, endIndex } = await jetV2.startEndScheduleIndex(0, scheduleTimes[0] + 400 * oneDay, scheduleTimes[3] + 2 * oneDay)
        const expectedScheduledReward1 = 
    ...
    it('should stake on behalf of another user', async () => {
        ...
        await auroraToken.connect(auroraOwner).approve(jetV2.address, amount)
        await expect(jetV2.connect(user1).stakeOnBehalfOfAnotherUser(
            user1.address,
            amount
        )).to.be.reverted
        await jetV2.connect(auroraOwner).stakeOnBehalfOfAnotherUser(
            user1.address,
            amount
        )
    ...
    it('should batch stake on bahalf of another users', async () => {
        ...
        await auroraToken.connect(auroraOwner).approve(jetV2.address, batchAmount)
        await expect(jetV2.connect(user1).stakeOnBehalfOfOtherUsers(
            [
                user1.address,
                user2.address
            ],
        ...
        await jetV2.connect(auroraOwner).stakeOnBehalfOfOtherUsers(
            [
                user1.address,
                user2.address
            ],
    ...
    it('should get user shares', async () => {
        expect(await jetV2.getUserShares(user1.address)).to.be.eq(0)
    })
    it('should get release time', async () => {
        ...
        await network.provider.send("evm_mine")
        await jetV2.connect(user1).unstake(amount)
        // get released time
        expect(parseInt(await jetV2.getReleaseTime(0, user1.address))).to.be.greaterThan(
    ...
    it('should withdraw rewards after release time', async () => {
         ...
         await jetV2.connect(user1).withdraw(streamId)
    ...
    it('should withdraw all rewards after release time', async () => {
        ...
        await streamToken1.connect(user1).approve(jetV2.address, maxRewardProposalAmountForAStream)
        await jetV2.connect(user1).createStream(Ids[1], maxRewardProposalAmountForAStream)
        await streamToken1.connect(user1).approve(jetV2.address, maxRewardProposalAmountForAStream)
        await jetV2.connect(user1).createStream(Ids[2], maxRewardProposalAmountForAStream)

        // stake
        const user2BalanceBefore = parseInt(await auroraToken.balanceOf(user2.address))
        await auroraToken.connect(user2).approve(jetV2.address, amount)
        await jetV2.connect(user2).stake(amount)
        await network.provider.send("evm_increaseTime", [101])
        await network.provider.send("evm_mine")

        // claim rewards
        await jetV2.connect(user2).batchMoveRewardsToPending([...Ids])
        expect(parseInt(await jetV2.getPending(Ids[0], user2.address))).to.be.greaterThan(0)

        // unstake
        await network.provider.send("evm_increaseTime", [1])
        await network.provider.send("evm_mine")
        await jetV2.connect(user2).unstake(amount)

        // withdraw batch and withdraw all
        await network.provider.send("evm_increaseTime", [tauPerStream + 1])
        await network.provider.send("evm_mine")
        await jetV2.connect(user2).batchWithdraw([0, 1])
        ...
        await jetV2.connect(user2).withdrawAll()
    ...
    it('should claim on behalf of another user', async() => {
        ...
        await jetV2.connect(stakingAdmin).claimOnBehalfOfAnotherUser(user2.address, Id)
        expect(parseInt(await jetV2.getPending(Id, user2.address))).to.be.greaterThan(0)
    })
    it('should batch claim on behalf of other users', async() => {
        ...
        await network.provider.send("evm_mine")
        // stake
        await auroraToken.connect(user3).approve(jetV2.address, amount)
        await jetV2.connect(user3).stake(amount)
        await network.provider.send("evm_increaseTime", [101])
        await network.provider.send("evm_mine")

        // claim rewards on behald of another user
        await jetV2.connect(stakingAdmin).batchClaimOnBehalfOfOtherUsers(
            [
                user2.address,
                user3.address,
            ],
        ...
        expect(parseInt(await jetV2.getPending(Ids[0], user2.address))).to.be.greaterThan(0)
        expect(parseInt(await jetV2.getPending(Ids[0], user3.address))).to.be.greaterThan(0)
        expect(parseInt(await jetV2.getPending(Ids[1], user2.address))).to.be.greaterThan(0)
        expect(parseInt(await jetV2.getPending(Ids[1], user3.address))).to.be.greaterThan(0)
    })
    it('should get zero stream owner claimable amount if stream is inactive', async() => {
        ...
        expect(await jetV2.getStreamOwnerClaimableAmount(Ids[0])).to.be.eq(0)
    })
    it('should return aurora stream user shares if stream is zero', async () => {
        ...
        // 
        expect(await jetV2.getAmountOfShares(0, user2.address)).to.be.eq(amount)
    ...
    it('should return total amount of staked aurora', async () => {
        ...
        const amount = ethers.utils.parseUnits("1000", 18)
         ...
         await network.provider.send("evm_mine")
         expect(
             parseInt(ethers.utils.formatEther(await jetV2.getTotalAmountOfStakedAurora()))
    ...
    it('should get zero reward amount before stream start and stream end', async () => {
        ...
        expect(await jetV2.getRewardsAmount(Ids[0], (await ethers.provider.getBlock("latest")).timestamp)).to.be.eq(0)
    ...
    it('should claim all rewards', async () => {
        ...
        await network.provider.send("evm_mine")
        await jetV2.connect(user1).moveAllRewardsToPending()
        expect(
            parseInt(await jetV2.getPending(id, user1.address))
        ).to.be.greaterThan(0)
    })
    it('should get reward per share for user', async () => {
        const id = 1
        expect(parseInt(await jetV2.getRewardPerShareForUser(id, user1.address))).to.be.eq(0)
         // deploy stream
        // approve aurora tokens to the stream proposal
        const auroraProposalAmountForAStream = ethers.utils.parseUnits("10000", 18)
        const maxRewardProposalAmountForAStream = ethers.utils.parseUnits("200000000", 18)
        ...
        expect(amount.mul(1024)).to.be.eq(
            await jetV2.getAmountOfShares(id, user1.address)
        )
    })
    it('should calculate weighted shares', async () => {
        ...
        expect(
            parseInt(
                await jetV2.calculateWeightedShares(shares, timestamp)
            )
    ...
    it('should get reward per share for a user', async () => {
        const id = 1
        const user1RPSBefore = parseInt(await jetV2.getRewardPerShareForUser(id, user1.address))
        ...
        await auroraToken.connect(user1).approve(jetV2.address, amount)
        await jetV2.connect(user1).stake(amount)
        const user1RPSAfter = parseInt(await jetV2.getRewardPerShareForUser(id, user1.address))
    ...
    it('should get claimable amount', async() => {
        ...
        const userRPS = await jetV2.getRewardPerShareForUser(id, user1.address)
        const latestRPS = await jetV2.getLatestRewardPerShare(id)
        const userShares = await jetV2.getAmountOfShares(id, user1.address)
        const expectedClaimableAmount = (latestRPS.sub(userRPS)).mul(userShares).div("1" + "0".repeat(31))
        expect(await jetV2.getStreamClaimableAmount(id, user1.address)).to.be.eq(expectedClaimableAmount)
    })
    it('should restake the rest of aurora tokens', async () => {
        ...
        // stake 
        const amount = ethers.utils.parseUnits("1000", 18)
        await auroraToken.connect(user5).approve(jetV2.address, amount)
        await jetV2.connect(user5).stake(amount)
        const userDepositBefore = parseInt(await jetV2.getUserTotalDeposit(user5.address))
        await network.provider.send("evm_increaseTime", [oneDay])
        await network.provider.send("evm_mine")
        const fiftyPercentOfShares = ethers.utils.parseUnits("500", 18)
        await jetV2.connect(user5).unstake(fiftyPercentOfShares)
        // get stream claimable amount
        const userRPS = parseInt(ethers.utils.formatEther(await jetV2.getRewardPerShareForUser(id, user5.address)))
        await network.provider.send("evm_increaseTime", [1000])
        await network.provider.send("evm_mine")
        const userDepositAfter = parseInt(await jetV2.getUserTotalDeposit(user5.address))
    ...
    it('should return zero total aurora staked if touchedAt equals zero', async () => {
        expect(
            await jetV2.getTotalAmountOfStakedAurora()
        ).to.be.eq(0)
    })
    it('should release rewards from stream start', async () => {
        ...
        await network.provider.send("evm_mine")
        expect(
            parseInt(await jetV2.getLatestRewardPerShare(id)) / 1e31
        ).to.be.greaterThan(0)
    })
    it('should calculate stream claimable rewards from stream start', async () => {
        ...
        await network.provider.send("evm_mine")
       expect(
            parseInt(await jetV2.getStreamClaimableAmount(id, user1.address))
        ).to.be.greaterThan(0)
    })
    it('should claim rewards for a stream even if user staked before stream deployment', async () => {
        ...
        expect(parseInt(ethers.utils.formatEther(await jetV2.getPending(id, user1.address)))).to.be.lessThanOrEqual(
    ...
    it('should admin remove stream', async () => {
        ...
        // create a stream
        await jetV2.connect(user1).createStream(id, maxRewardProposalAmountForAStream)
        await jetV2.connect(streamManager).removeStream(id, user5.address)
    ...
    it('should admin cancel stream proposal after expiry date', async() => {
        ...
        await jetV2.connect(streamManager).cancelStreamProposal(id)
        const stream = await jetV2.getStream(id)
    ...
    it('admin can claim streams on behalf of another user', async() => {
        ...
        await jetV2.connect(pauser).batchClaimOnBehalfOfAnotherUser(
            user2.address,
            [Ids[0], Ids[2]]
        )
        expect(parseInt(await jetV2.getPending(Ids[0], user2.address))).to.be.greaterThan(0)
        expect(parseInt(await jetV2.getPending(Ids[1], user2.address))).to.be.eq(0)
        expect(parseInt(await jetV2.getPending(Ids[2], user2.address))).to.be.greaterThan(0)
    })
    it('estimageGas staking with multiple streams', async () => {
        ...
        await auroraToken.connect(user1).approve(jetV2.address, amount)
        let tx = await jetV2.connect(user1).stake(amount)
        ...
        tx = await jetV2.connect(user1).stake(amount)
        ...
        await auroraToken.connect(user2).approve(jetV2.address, amount)
        tx = await jetV2.connect(user2).stake(amount)
    ...
    it('estimageGas claiming all with multiple users', async () => {
        ...
        await auroraToken.connect(auroraOwner).mint(auroraOwner.address, batchAmount)
        await auroraToken.connect(auroraOwner).approve(jetV2.address, batchAmount)
        let tx = await jetV2.connect(auroraOwner).stakeOnBehalfOfOtherUsers(users, amounts, batchAmount)
        console.log("Airdrop stake 1st time:", (await tx.wait()).cumulativeGasUsed.toNumber(), "gas.")
        await network.provider.send("evm_increaseTime", [10])
        await network.provider.send("evm_mine")

        // airdrop claim before stake
        tx = await jetV2.connect(auroraOwner).claimAllOnBehalfOfOtherUsers(users)
        ...
        await auroraToken.connect(auroraOwner).mint(auroraOwner.address, batchAmount)
        await auroraToken.connect(auroraOwner).approve(jetV2.address, batchAmount)
        tx = await jetV2.connect(auroraOwner).stakeOnBehalfOfOtherUsers(users, amounts, batchAmount)
        console.log("Airdrop add stake:", (await tx.wait()).cumulativeGasUsed.toNumber(), "gas.")
        await network.provider.send("evm_increaseTime", [10])
        await network.provider.send("evm_mine")

        tx = await jetV2.connect(auroraOwner).claimAllOnBehalfOfAnotherUser(users[0])
    ...
    it('should not return zero stake value when a user unstakeAll', async () => {
        ...
        await auroraToken.connect(user4).approve(jetV2.address, amountStaked)
        await jetV2.connect(user4).stake(amountStaked)
        ...
        await auroraToken.connect(user5).approve(jetV2.address, amountStaked)
        await jetV2.connect(user5).stake(amountStaked)
        // user 4 unstakeAll 
        console.log(`user 1 shares: ${ethers.utils.formatEther(await jetV2.getUserShares(user4.address))}`)
        console.log(`user 2 shares: ${ethers.utils.formatEther(await jetV2.getUserShares(user5.address))}`)
        // user 5 trying to unstakeAll
        console.log(`user 1 unstakeAll`)
        await jetV2.connect(user4).unstakeAll()
        console.log(`user 2 unstakeAll`)
        await jetV2.connect(user5).unstakeAll()
         // withdraw
         await network.provider.send("evm_increaseTime", [tauPerStream + 1])
         await network.provider.send("evm_mine")
         const streamId = 0 // main aurora rewards
         await jetV2.connect(user4).withdraw(streamId)
         await jetV2.connect(user5).withdraw(streamId)
         ...
         console.log(`total user shares ${await jetV2.totalAuroraShares()}`)
    ...
    it('should user 1 stakes before user 2 but both stake very small but the same amount and unstake at the same time', async () => {
        ...
        await auroraToken.connect(user4).approve(jetV2.address, amountStaked)
        await jetV2.connect(user4).stake(amountStaked)
        await jetV2.connect(user4).updateUserCalculation()
        console.log(
            'total amount of AURORA staked',
            ethers.utils.formatEther(
                await jetV2.totalAmountOfStakedAurora()
            )
        )
        // user 5 stakes (1**-17) Aurora same amount but after 1 second.
        console.log(`user 2 stakes: ${ethers.utils.formatEther(amountStaked)} AURORA`)
        ...
        await jetV2.connect(user4).unstakeAllOnBehalfOfOthers([user4.address, user5.address])
         // withdraw
         await network.provider.send("evm_increaseTime", [tauPerStream + 1])
         await network.provider.send("evm_mine")
         const streamId = 0 // main aurora rewards
    ...
    it('should both users get the same reward if they stake and unstake the same amount at the same time', async() => {
        ...
        console.log(`user 1 stakes: ${ethers.utils.formatEther(amountStaked)} AURORA`)
        const user1BalanceBefore = ethers.utils.formatEther(await auroraToken.balanceOf(user4.address))
        await auroraToken.connect(user1).approve(jetV2.address, totalAmount)
        await jetV2.connect(user1).stakeOnBehalfOfOtherUsers(
            [
                user4.address,
                user5.address
            ],
    ...
    it('should return the right share calculations', async () => {
        ...
        console.log('User 1 shares: ',
            ethers.utils.formatEther(
                await jetV2.getUserShares(user4.address)
            )
        )
        ...
        console.log('User 2 shares: ',
            ethers.utils.formatEther(
                await jetV2.getUserShares(user5.address)
            )
        )
        // user 4 unstakeAll 
        // user 5 trying to unstakeAll
    ...
    it('should user 0 stake, then two new users stake and unstake the same amount at the same time', async() => {
        ...
        const amountStaked = ethers.utils.parseUnits("1", 18)
        await auroraToken.connect(user3).approve(jetV2.address, amountStaked)
        await jetV2.connect(user3).stake(amountStaked)
        console.log(`user 1 stakes: ${ethers.utils.formatEther(amountStaked)} AURORA`)
        const user1BalanceBefore = ethers.utils.formatEther(await auroraToken.balanceOf(user4.address))
        await auroraToken.connect(user1).approve(jetV2.address, totalAmount)
        await jetV2.connect(user1).stakeOnBehalfOfOtherUsers(
    ...
    it('should not have a possible race condition', async () => {
        ...
        await auroraToken.connect(user1).approve(jetV2.address, totalUsersStake)
        await jetV2.connect(user1).stakeOnBehalfOfOtherUsers(
            [
                user4.address,
                user5.address
