...
describe("JetStakingV1", function () {
    ...
    it('user stakes and never claims', async () => {
        ...
        await network.provider.send("evm_increaseTime", [4 * oneYear - timeDiff -10]) // increase time for 20 days
    ...
    it('should multiple users stake and unstake over 4 years', async () => {
        ...
        console.log(parseFloat(ethers.utils.formatEther(await jet.totalAmountOfStakedAurora())))
        ...
        // expect(parseFloat(ethers.utils.formatEther(await jet.connect(user1).getTotalUserReward()))).to.be.lessThanOrEqual(
        ...
        timeAfter = (await ethers.provider.getBlock("latest")).timestamp
        timeDiff = timeAfter - timeBefore
        let expectedRewardDuringThisPeriod = (scheduleRewards[1] - scheduleRewards[2]) * 2 * oneDay / (1e18 * oneYear) + 
            (scheduleRewards[2] - scheduleRewards[3]) * (timeDiff - 2 * oneDay) / (1e18 * oneYear)
        totalAmountOfStakedAurora += expectedRewardDuringThisPeriod
        // console.log(
        //     totalAmountOfStakedAurora, 
        //     parseFloat(ethers.utils.formatEther(await jet.totalAmountOfStakedAurora()))
        // )
        p.printTable();
    })
